...
var getTextContent = function(ele){
  var parser = new DOMParser;
    var dom = parser.parseFromString(
        '<!doctype html><body>' + ele.textContent,
        'text/html');
...
NlmToLensConverter.Prototype = function() {
  ...
  this.extractDate = function(dateEl) {
    ...
    if (dateEl.getAttribute('iso-8601-date'))
      return dateEl.getAttribute('iso-8601-date');
    else
  ...
  this.extractCustomMetaGroup = function(state, article) {
    ...
    for (var i = 0; i < customMetaEls.length; i++) {
      ...
      if (!_.include(this.__ignoreCustomMetaNames, getTextContent(metaNameEl))) {
        var header = {
          "type" : "heading",
          "id" : state.nextId("heading"),
          "level" : 3,
    ...
    if (license) {
      ...
      if (copyright) {
        ...
        if (par && par.length) {
          ...
          if (getTextContent(copyright).trim().slice(-1) !== '.') {
  ...
  this.contribGroup = function(state, contribGroup) {
    ...
    if (onBehalfOf) doc.on_behalf_of = getTextContent(onBehalfOf).trim();
  };
  ...
  this.affiliation = function(state, aff) {
    ...
    var affiliationNode = {
      ...
      label: label ? getTextContent(label) : null,
  ...
  this.extractContributorProperties = function(state, contrib, contribNode) {
    ...
    _.each(xrefs, function(xref) {
      if (xref.getAttribute("ref-type") === "aff") {
        ...
        var fundingSourceName = getTextContent(fundingSource.childNodes[0]);
        ...
        if (!email) return;
        contribNode.emails.push(getTextContent(email));
        ...
        if (fnElem) {
          ...
          switch (fnType) {
            case "con":
              contribNode.contribution = getTextContent(fnElem);
              break;
            case "conflict":
  ...
  this.extractDefinitions = function(state /*, article*/) {
    ...
    _.each(defItems, function(defItem) {
      ...
      var definitionNode = {
        ...
        description: getTextContent(def)
      };
  ...
  this.abstract = function(state, abs) {
    ...
    var heading = {
      ...
      type: "heading",
      level: 1,
      content: title ? getTextContent(title) : "Abstract"
    };
  ...
  this.section = function(state, section) {
    ...
    if (nodes.length > 0 && title) {
      ...
      if (label) {
        heading.label = getTextContent(label);
      }
  ...
  this.figure = function(state, figure) {
    ...
    if (attrib) {
      ...
      figureNode.attrib = getTextContent(attrib);
    }
  ...
  this.supplement = function(state, supplement) {
    ...
    var supplementNode = {
      ...
      "type": "supplement",
      "label": label ? getTextContent(label) : "",
      "url": url,
      "caption": null
    };
  ...
  this.video = function(state, video) {
    var doc = state.doc;
    var label = getTextContent(video.querySelector("label"));
  ...
  this.tableWrap = function(state, tableWrap) {
    ...
    var tableNode = {
      ...
      "type": "html_table",
      "title": "",
      "label": label ? getTextContent(label) : "Table",
      "content": "",
      "caption": null,
  ...
  this.formula = function(state, formulaElement, inline) {
    ...
    if (label) formulaNode.label = getTextContent(label);
  ...
  this.citation = function(state, ref, citation) {
    ...
    if (personGroup) {
      ...
      for (i = 0; i < collabElements.length; i++) {
        getTextContent(citationNode.authors.push(collabElements[i]));
      }

      var source = citation.querySelector("source");
      if (source) citationNode.source = getTextContent(source);
      ...
      if (volume) citationNode.volume = getTextContent(volume);

      var publisherLoc = citation.querySelector("publisher-loc");
      if (publisherLoc) citationNode.publisher_location = getTextContent(publisherLoc);

      var publisherName = citation.querySelector("publisher-name");
      if (publisherName) citationNode.publisher_name = getTextContent(publisherName);

      var fpage = citation.querySelector("fpage");
      if (fpage) citationNode.fpage = getTextContent(fpage);

      var lpage = citation.querySelector("lpage");
      if (lpage) citationNode.lpage = getTextContent(lpage);

      var year = citation.querySelector("year");
      if (year) citationNode.year = getTextContent(year);

      // Note: the label is child of 'ref'
      var label = ref.querySelector("label");
      if(label) citationNode.label = getTextContent(label);
  ...
  this.addAnnotationData = function(state, anno, el, type) {
    ...
    if (type === 'xref') {
      ...
      anno.url = "mailto:" + getTextContent(el).trim();
    } else if (type === 'inline-graphic') {
      anno.url = el.getAttribute("xlink:href");
    } else if (type === 'inline-formula') {
  ...
  this._annotatedText = function(state, iterator, options) {
    ...
    while(iterator.hasNext()) {
      ...
      if (el.nodeType === Node.TEXT_NODE) {
        var text = state.acceptText(getTextContent(el));
        plainText += text;
        charPos += text.length;
      }
      // Annotations...
